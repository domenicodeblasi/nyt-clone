...
const FirstOpinionsSection = ({ opinionsArray, opinion }) => {
    const { title, byline, multimedia, url } = opinion
    return (
        <a href={url} title={title} className="cursor-pointer">
        <section className={`flex gap-2 py-4 border-b border-gray-400 ${opinion === opinionsArray[1] && "md:border-0 lg:border-b"}`}>
            <div className={`flex flex-col gap-1 ${opinion === opinionsArray[1] && "basis-1/2"}`}>
