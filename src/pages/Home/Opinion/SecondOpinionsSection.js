...
const SecondOpinionsSection = ({ opinionsArray, singleOpinion }) => {
    const { title, byline, url } = singleOpinion
    return (
        <a href={url} title={title} className="cursor-pointer">
            <section className={`flex flex-col gap-1 py-4 border-b border-gray-400 ${singleOpinion === opinionsArray[1] && "md:border-0 lg:border-b"}`}>
