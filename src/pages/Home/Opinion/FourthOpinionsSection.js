...
const FourthOpinionsSection = ({ opinionsArray, opinion }) => {
    const { title, byline, url } = opinion
    return (
        <a href={url} title={title} className="cursor-pointer">
        <section className={`flex flex-col gap-1 py-4 border-b border-gray-400 ${opinion === opinionsArray[2] && "md:border-0 lg:border-b"}`}>
